...
contains
    ...
    !  Parameters:
    !
    !    Input, integer (int32) NCC, the number of constraint curves
    !    (constraint regions).  0 <= NCC.
    !
    !    Input, integer (int32) LCC(NCC) (or dummy array of length 1 if
    ...
    !
    !    Input, integer (int32) N, the number of nodes in the triangulation,
    !    including constraint nodes.  3 <= N.
    !
    !    Input, real (real64) X(N), Y(N), the coordinates of the nodes with
    ...
    !
    !    Input/output, integer (int32) LWK.  On input, the length of IWK.
    ...
    !
    !    Output, integer (int32) IWK(LWK), the endpoint indexes of the new
    !    arcs which were swapped in by the last call to subroutine EDGE.
    !
    !    Input/output, integer (int32) LIST(*), LPTR(*), LEND(N), the data
    ...
    !
    !    Output, integer (int32) IER = Error indicator:
    ...
    !
      ...
      integer (int32) ifrst
      integer (int32) ilast
      integer (int32) iwk(lwk)
      integer (int32) k
      integer (int32) kbak
      integer (int32) kfor
      integer (int32) kn
      integer (int32) lcc(*)
      integer (int32) lccip1
    ...
    !  Parameters:
    !
    !    Input, integer (int32) K, the nodal index (index for X, Y, and LEND)
    !    of the new node to be added.  1 <= K <= LCC(1).  (K <= N+1 if NCC=0).
    !
    !    Input, real (real64) XK, YK, the coordinates of the new node (to be
    !    stored in X(K) and Y(K)).  The node must not lie in a constraint region.
    !
    !    Input, integer (int32) IST, the index of a node at which TRFIND
    ...
    !    0 <= NCC.
    !
    !    Input/output, integer (int32) LCC(*), list of constraint curve
    ...
    !
    !    Input/output, integer (int32) N, the number of nodes in the
    !    triangulation.  3 <= N.  Note that N will be incremented following the
    !    addition of node K.
    !
    !    Input, real (real64) X(N+1), real Y(N+1), containing the coordinates
    ...
    !
    !    Input/output, integer (int32) LIST(*), LPTR(*), LEND(N), LNEW, the
    ...
    !  Parameters:
    !
    !    Input, real (real64) X(*), Y(*), the Cartesian coordinates of a set
    !    of points.
    !
    !    Input, integer (int32) NB, the number of points in the curve.
    !
    !    Input, integer (int32) NODES(NB), the indices of the points that
    !    make up the closed curve.
    !
    !    Output, real (real64) AREAP, the signed area bounded by the curve.
    !
      implicit none
    ...
    !  Parameters:
    !
    !    Input, integer (int32) KK, the index of a node to be connected to the
    !    sequence of all visible boundary nodes.  1 <= KK and
    !    KK must not be equal to I1 or I2.
    !
    !    Input, integer (int32) I1, the first (rightmost as viewed from KK)
    !    boundary node in the triangulation which is visible from
    !    node KK (the line segment KK-I1 intersects no arcs.
    !
    !    Input, integer (int32) I2, the last (leftmost) boundary node which is
    ...
    !    3 <= N.
    !
    !    Input, integer (int32) LIST(*), LPTR(*), LEND(N), the data structure
    !    defining the triangulation.  Refer to subroutine TRMESH.
    !
    !    Output, integer (int32) NODES(NB), ordered sequence of boundary node
    !    indexes in the range 1 to N.
    !
    !    Output, integer (int32) NB, the number of boundary nodes.
    !
    !    Output, integer (int32) NA, NT, the number of arcs and triangles,
    ...
    !    to be computed.
    !
    !    Output, real (real64) XC, YC, coordinates of the circumcenter (center
    !    of the circle defined by the three points) unless SA = 0, in which XC
    !    and YC are not altered.
    !
    !    Output, real (real64) CR, the circumradius (radius of the circle
    !    defined by the three points) unless SA = 0 (infinite radius), in which
    !    case CR is not altered.
    !
    !    Output, real (real64) SA, the signed triangle area with positive value
    ...
    !    toward (X2,Y2).
    !
    !    Output, real (real64) AR, the aspect ratio r/CR, where r is the
    ...
    !  Parameters:
    !
    !    Input, integer (int32) NCC, LCC(*), onstraint data structure.
    !    Refer to ADDCST.
    !
    !    Input, integer (int32) I1, I2, I3, Nodal indexes of the counterclockwise-
    ...
    !    4 <= N.
    !
    !    Input, integer (int32) IO1, IO2, the indexes (in the range 1 to N) of
    ...
    !  Parameters:
    !
    !    Input, integer (int32) N0, NB, indexes, in the range 1 to N, of a
    ...
    !
    !    Output, integer (int32) LPH, list pointer to the hole (NB as a
    ...
    !  Parameters:
    !
    !    Input, integer (int32) K, the index (for X and Y) of the node to be
    ...
    !    deletion.
    !
    !    Input/output, real (real64) X(N), Y(N), the coordinates of the nodes
    ...
    !
    !    Input/output, integer (int32) LWK.  On input, the number of columns
    ...
    !    IER = 1 or IER = 3.
    !
    !    Output, integer (int32) IWK(2,LWK), indexes of the endpoints of the
    ...
    !  Parameters:
    !
    !    Input, integer (int32) IN1, IN2, indexes (of X and Y) in the range
    ...
    !    (or LWK=0) on input.
    !
    !    Output, integer (int32) IWK(2*LWK), the indexes of the endpoints of
    ...
    !  Parameters:
    !
    !    Input, integer (int32) NCC, the number of constraints.  NCC >= 0.
    !
    !    Input, integer (int32) LCC(*), a list of constraint curve starting
    ...
    !
    !    Input, integer (int32) LIST(*), LPTR(*), LEND(N), the triangulation
    !    data structure.  Refer to subroutine TRMESH.
    !
    !    Input, integer (int32) L, the number of nodes in the sequence
    !    on output.  2 <= L <= N.
    !
    !    Input/output, integer (int32) NPTS(L), on input, the indexes of the
    ...
    !    unless IER /= 0.
    !
    !    Input/output, real (real64) DS(L), the distance (defined above)
    ...
    !    Input, integer (int32) NCC, the number of constraints.  NCC >= 0.
    !
    !    Input, integer (int32) LCC(*), list of constraint curve starting
    !    indexes (or dummy array of length 1 if NCC = 0).  Refer to subroutine
    !    ADDCST.
    !
    !    Input, integer (int32) N, the number of nodes in the triangulation.
    !    N >= 3.
    !
    !    Input, integer (int32) LIST(*), LEND(N), the data structure defining
    !    the triangulation.  Refer to subroutine TRMESH.
    !
    !    Output, integer (int32) INDXCC, index of the exterior constraint
    ...
    !  Parameters:
    !
    !    Input, integer (int32) K, the index of the node to be inserted.
    !
    !    Input, integer (int32) LP, the LIST pointer of N2 as a neighbor of N1.
    !
    !    Input/output, integer (int32) LIST(*), LPTR(*), LNEW, the data
    !    structure defining the triangulation.  Refer to subroutine TRMESH.  On
    ...
    !  Parameters:
    !
    !    Input, integer (int32) KK, the index of the node to be inserted.
    !    1 <= KK and KK must not be equal to I1, I2, or I3.
    !
    !    Input, integer (int32) I1, I2, I3, indexes of the
    ...
    !  Parameters:
    !
    !    Input, real (real64) X1, Y1 = Coordinates of P1.
    !
    !    Input, real (real64) X2, Y2 = Coordinates of P2.
    !
    !    Input, real (real64) X3, Y3 = Coordinates of P3.
    !
    !    Input, real (real64) X4, Y4 = Coordinates of P4.
    !
    ...
    !  Parameters:
    !
    !    Input, integer (int32) N, the maximum value to be returned.
    !
    !    Input/output, integer (int32) IX, IY, IZ, seeds initialized to values
    ...
    !
    !    Output, integer (int32) JRAND, random integer in the range 1 to N.
    !
    !  Local parameters:
    !
    ...
    !  Parameters:
    !
    !    Input, real (real64) X1, Y1, coordinates of N1.
    !
    !    Input, real (real64) X2, Y2, coordinates of N2.
    !
    !    Input, real (real64) X0, Y0, coordinates of N0.
    !
    ...
    !  Parameters:
    !
    !    Input, integer (int32) LPL = LEND(N0).
    !
    !    Input, integer (int32) NB, the index of the node whose pointer is to
    !    be returned.  NB must be connected to N0.
    !
    !    Input, integer (int32) LIST(*), LPTR(*), the data structure defining
    !    the triangulation.  Refer to subroutine TRMESH.
    !
    !    Output, integer (int32) LSTPTR, pointer such that LIST(LSTPTR) = NB or
    ...
    !  Parameters:
    !
    !    Input, integer (int32) LPL, the LIST pointer to the last neighbor
    !    of N0.  LPL = LEND(N0).
    !
    !    Input, integer (int32) LPTR(*), pointers associated with LIST.
    !
    !    Output, integer (int32) NBCNT, the  number of neighbors of N0.
    !
      implicit none
    ...
    !  Parameters:
    !
    !    Input, real (real64) XP, YP, the coordinates of the point P.
    !
    !    Input, integer (int32) IST, the index of a node at which TRFIND begins
    ...
    !    Input, real (real64) X(N), Y(N), the coordinates of the nodes.
    !
    !    Input, integer (int32) LIST(*), LPTR(*), LEND(N), a data structure
    !    defining the triangulation.  Refer to TRMESH.
    !
    !    Output, real (real64) DSQ, the square of the distance between P and
    !    node NEARND.
    !
    !    Output, integer (int32) NEARND, the index of the nearest node to P,
    !    or 0 if N < 3 or the triangulation data structure is invalid.
    !
      implicit none

      integer (int32), parameter :: lmax = 25
      integer (int32) n
      ...
      integer (int32) lend(n)
      integer (int32) list(*)
      integer (int32) listp(lmax)
      integer (int32) lp
      integer (int32) lp1
      integer (int32) lp2
      integer (int32) lpl
      integer (int32) lptr(*)
      integer (int32) lptrp(lmax)
      ! integer (int32) lstptr
      integer (int32) n1
      integer (int32) n2
      integer (int32) n3
      integer (int32) nearnd
      integer (int32) nr
      integer (int32) nst
      real (real64) sin1
      real (real64) sin2
    ...
    !  Parameters:
    !
    !    Input, real (real64) X(*), Y(*), the nodal coordinates.
    !
    !    Input, integer (int32) NA, the number of arcs in the set.  0 <= NA.
    !
    !    Input/output, integer (int32) LIST(*), LPTR(*), LEND(N), data
    !    structure defining the triangulation.  Refer to subroutine TRMESH.
    !    On output, updated to reflect the swaps.
    !
    !    Input/output, integer (int32) NIT.  On input, the maximum number of
    !    iterations to be performed.  A reasonable value is 3*NA.  1 <= NIT.  On
    !    output, the number of iterations performed.
    !
    !    Input/output, integer (int32) IWK(2,NA), containing the nodal indexes
    ...
    !  Parameters:
    !
    !    Input, real (real64) X, the value to be stored.
    !
    !    Output, real (real64) STORE, the value of X after it has been stored
    ...
    !  Parameters:
    !
    !    Input, integer (int32) IN1, IN2, IO1, IO2, the nodal indexes of the
    ...
    !
    !    Output, integer (int32) LP21, the index of IN1 as a neighbor of IN2
    ...
    !  Parameters:
    !
    !    Input, integer (int32) NST, the index of a node at which TRFIND begins
    !    the search.  Search time depends on the proximity of this node to P.
    !
    !    Input, real (real64) PX, PY, the coordinates of the point P to be
    !    located.
    !
    !    Input, integer (int32) N, the number of nodes in the triangulation.
    !    3 <= N.
    !
    !    Input, real (real64) X(N), Y(N), the coordinates of the nodes in
    !    the triangulation.
    !
    !    Input, integer (int32) LIST(*), LPTR(*), LEND(N), the data structure
    !    defining the triangulation.  Refer to subroutine TRMESH.
    !
    !    Output, integer (int32) I1, I2, I3, nodal indexes, in counterclockwise
    ...
    !
      ...
      integer (int32) i2
      integer (int32) i3
      integer (int32), save :: ix = 1
      integer (int32), save :: iy = 2
      integer (int32), save :: iz = 3
      ! integer (int32) jrand
      ! logical left
      integer (int32) lend(n)
      integer (int32) list(*)
    ...
    !    N >= 3.
    !
    !    Input, integer (int32) LIST(*), LPTR(*), LEND(N), linked list data structure
    !    defining the triangulation.  Refer to subroutine TRMESH.
    !
    !    Input, integer (int32) NROW, the number of rows (entries per triangle)
    ...
    !    Refer to LTRI.
    !
    !    Input, integer (int32) LTRI(NROW*NT), where NT is at most 2N-5.  (A
    !    sufficient length is 12 * N if NROW=6 or 18*N if NROW=9.)
    !
    !    Output, integer (int32) NT, the number of triangles in the
    !    triangulation unless IER /= 0, in which case NT = 0.  NT = 2N - NB- 2,
    !    where NB is the number of boundary nodes.
    !
    !    Output, integer (int32) LTRI(NROW,NT), whose J-th column contains the vertex nodal
    ...
    !
    !    Output, integer (int32) LCT(NCC), containing the triangle index of the first
    ...
    !  Parameters:
    !
    !    Input, integer (int32) NCC, the number of constraints.
    !
    !    Input, integer (int32) LCT(NCC), the list of constraint triangle
    !    starting indexes (or dummy array of length 1 if NCC = 0).
    !
    !    Input, integer (int32) N, the number of nodes in the triangulation.
    !    3 <= N <= 9999.
    !
    !    Input, real (real64) X(N), Y(N), the coordinates of the nodes in the
    ...
    !
    !    Input, integer (int32) NT, the number of triangles in the
    !    triangulation.  1 <= NT <= 9999.
    !
    !    Input, integer (int32) LTRI(NROW,NT), array whose J-th column contains
    ...
    !
      ...
      integer (int32) k
      integer (int32) lct(*)
      integer (int32) ltri(nrow,nt)
      integer (int32) na
      integer (int32) nb
      integer (int32) nl
      integer (int32), parameter :: nlmax = 60
      integer (int32), parameter :: nmax = 9999
      integer (int32) ncc
      logical prntx
      real (real64) x(n)
      real (real64) y(n)
    ...
    !
    !    Output, integer (int32) LIST(6*N-12), nodal indexes which, along with LPTR,
    ...
    !    of its index.
    !
    !    Output, integer (int32) LPTR(6*N-12), pointers (LIST indexes) in
    ...
    !
    !    Output, integer (int32) LEND(N), pointers to adjacency lists.  LEND(K)
    !    points to the last neighbor of node K for K = 1,...,N.  Thus,
    !    LIST(LEND(K)) < 0 if and only if K is a boundary node.
    !
    !    Output, integer (int32) LNEW, pointer to the first empty location in LIST
    ...
    !  Parameters:
    !
    !    Input, integer (int32) N, the number of nodes in the grid.  N = NX*NY
    !    for some NY >= 2.
    !
    !    Input, integer (int32) NX, the number of grid points in the x-direction.
    !    NX >= 2.
    !
    !    Input, real (real64) X(N), Y(N), the coordinates of the nodes with
    ...
    !
    !    Input/output, integer (int32) NIT.  On input, the maximum number of iterations
    !    to be employed.  Refer to the header comments above.  On output,
    !    the actual number of iterations.
    !
    !    Output, integer (int32) LIST(6*N-12), LPTR(6*N-12), LEND(N), LNEW,
    !    data structure defining the triangulation.  Refer to subroutine TRMESH.
    !
    !    Output, integer (int32) IER = Error indicator:
    !     0 if no errors were encountered.
    ...
    !  Parameters:
    !
    !    Input, integer (int32) N, the number of nodes.  N .GE. 3.
    !
    !    Input, real (real64) X(N), Y(N), the nodal coordinates.
    !
    !    Input, integer (int32) LIST(*), LPTR(*), LEND(N), the data structure
    !    containing the triangulation.  Refer to subroutine TRMESH.
    !
    !    Input, real (real64) TOL, nonnegative tolerance to allow for
    ...
    !    to be 0.
    !
    !    Output, real (real64) ARMAX, maximum aspect ratio (radius of inscribed
    ...
    12 continue
      ...
      ier = 2
      write (*,120) n2, lp, n1
      120 format (//5x,'*** trmtst -- list(lp) =',i0, &
              ', for lp =',i0,','/19x, &
      'is not a valid neighbor of ',i0,' ***'/)
      return
    !
    ...
    !  Parameters:
    !
    !    Input, integer (int32) LUN, the logical unit number in the range
    !    0 to 99.  The unit should be opened with an appropriate
    !    file name before the call to this routine.
    !
    !    Input, real (real64) PLTSIZ, the plot size in inches.  The window is
    ...
    !
    !    Input, real (real64) WX1, WX2, WY1, WY2, parameters defining a
    ...
    !
    !    Input, integer (int32) NCC, the number of constraint curves.  Refer to
    !    subroutine ADDCST.  NCC >= 0.
    !
    !    Input, integer (int32) LCC(NCC) (or dummy parameter if NCC = 0)
    ...
    !
    !    Input, integer (int32) LIST(*), LPTR(*), LEND(N), data structure
    !    defining the triangulation.  Refer to subroutine TRMESH.
    !
    ...
    !
      ...
      logical cnstr
      real (real64), parameter :: dashl = 4.0D+00
      real (real64) dx
      real (real64) dy
      real (real64), parameter :: fsizn = 10.0D+00
      real (real64), parameter :: fsizt = 16.0D+00
      integer (int32) i
      integer (int32) ier
      integer (int32) ifrst
      integer (int32) ih
    ...
    !
      iw = nint ( 0.88D+00 * dble ( ipx2 - ipx1 ) )
      ih = nint ( 0.88D+00 * dble ( ipy2 - ipy1 ) )
      ipx1 = 306 - iw/2
      ipx2 = 306 + iw/2
      ipy1 = 396 - ih/2
      ipy2 = 396 + ih/2
