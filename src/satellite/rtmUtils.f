module rtmUtils_mod
    ...
    subroutine doRtmForward(pinfo,platform,state,rtmObsOp,obs,startx,endx,starty,endy,&
        ...
        class(SatelliteObservation), pointer :: obsSubset => null()
        ...
        class(DataDimension), pointer :: mobsDim, nobsDim, nLociDim
        class(DataVariable),  pointer :: obsDataVar, obsLociVar
        ...
        do batch=1,numBatch
            ...
            mobsDim    => obsSubset%addDimensionByName(MOBS_DIM_NAME, nchan)
            nobsDim    => obsSubset%addDimensionByName(NOBS_DIM_NAME,nxl*ny)
            nLociDim   => obsSubset%addDimensionByName(NLOCI_DIM_NAME,4)
            obsDataVar => obsSubset%addVariable(pinfo,OBS_DATA_VAR_NAME,obsDataLocal,&
                mobsDim,nobsDim)
            obsLociVar => obsSubset%addVariable(pinfo,OBS_LOCI_VAR_NAME,obsLociLocal,&
                nlociDim,nobsDim)
            ...
            do y=1,ny
                do xl=1,nxl
                    profile = profile + 1
                    x = xs_batch+xl-1
                    obsLociLocal(1,profile) = lat(x,y)
                    obsLociLocal(2,profile) = lon(x,y)
                    obsLociLocal(3,profile) = x + startxval - 1
                    obsLociLocal(4,profile) = y + startyval - 1
                end do
            end do

            call obsSubset%loadSatelliteObservation(pinfo,obsDataVar,obsLociVar)

            call rtmObsOp%doForward(state, obsSubset, obsDataLocal)
...
!
!        class(SatelliteObservationOperator),  pointer :: rtmObsOp
!        class(SatelliteObservation),          pointer :: satObs, satObs_tl
!
...
!        real(real64), pointer :: obsLoci(:,:)
!
!        real(real64), pointer :: obsData_tl(:,:)
!        real(real64), pointer :: obsLoci_tl(:,:)
!
!        real(real64), pointer :: obsLociP(:,:)
!
...
!        class(DataVariable),  pointer :: obsDataTlVar
!        class(DataVariable),  pointer :: obsLociTlVar
!
!        class(ParallelInfo),  pointer :: pinfo
!
!        allocate(pinfo)
...
!
!            nobsDim  => satObs%addDimensionByName(NOBS_DIM_NAME,1)
!            mobsDim  => satObs%addDimensionByName(MOBS_DIM_NAME,platform%getNChannels())
!            nLociDim => satObs%addDimensionByName(NLOCI_DIM_NAME,4)
!
!            obsDataVar => satObs%addVariable(pinfo,OBS_DATA_VAR_NAME,obsData,&
!                mobsDim,nobsDim)
!            obsLociVar => satObs%addVariable(pinfo,OBS_LOCI_VAR_NAME, obsLoci, &
!                & nLociDim, nobsDim)
!
!            obsLoci(1,1) = state_ptr%getValue2D(A3D_LAT_VAR,x,y)
!            obsLoci(2,1) = state_ptr%getValue2D(A3D_LON_VAR,x,y)
!            obsLoci(3,1) = x
!            obsLoci(4,1) = y
!
!            call satObs%loadSatelliteObservation(pinfo,obsDataVar,obsLociVar)
!
!            call satObs_tl%satelliteObservationConstructor(platform)
!
!            nobsDim  => satObs_tl%addDimensionByName(NOBS_DIM_NAME,1)
!            mobsDim  => satObs_tl%addDimensionByName(MOBS_DIM_NAME,platform%getNChannels())
!            nLociDim => satObs_tl%addDimensionByName(NLOCI_DIM_NAME,4)
!
!            obsDataTlVar => satObs_tl%addVariable(pinfo,OBS_DATA_VAR_NAME,obsData_tl,&
!                mobsDim,nobsDim)
!            obsLociTlVar => satObs_tl%addVariable(pinfo,OBS_LOCI_VAR_NAME, obsLoci_tl, &
!                & nLociDim, nobsDim)
!
!            obsLoci_tl(1,1) = state_ptr%getValue2D(A3D_LAT_VAR,x,y)
!            obsLoci_tl(2,1) = state_ptr%getValue2D(A3D_LON_VAR,x,y)
!            obsLoci_tl(3,1) = x
!            obsLoci_tl(4,1) = y
!
!            call satObs_tl%loadSatelliteObservation(pinfo,obsDataTlVar,obsLociTlVar)
!
!            zExtent => state_ptr%getBottomTopExtent()
!
!            nz = zExtent%getLocalCount()
...
!
!            obsLoci  => satObs%getObsLoci()
!            obsLociP => satObs%getObsData()
!
!            obsLoci(SO_PIX_DIM,1) = x
!            obsLoci(SO_SCAN_DIM,1) = y
!
...
!
!                call rtmObsOp%doTLM(state_ptr, satObs, perturb_ptr, satObs_tl%getObsData())
!                optr => obsLociP(:,1)
...
!            deallocate(obsy)
!            deallocate(satObs)
!            deallocate(satObs_tl)
!            deallocate(obsDataVar)
!            deallocate(obsLociVar)
!            deallocate(dptr)
!            deallocate(dptr2)
