...
use datetime_timedelta_mod, only:timedelta
use datetime_strftime_mod, only:tm_struct,c_strftime,c_strptime
use datetime_constants_mod
...
pure elemental real(kind=real64) function date2num(d)
  ...
  !! Given a datetime instance d, returns Julian time.
  !! This is taken from http://fortranwiki.org/fortran/show/m_time as the
  !! original implementation involved a very crude loop over the number of years.

  type(datetime),intent(in) :: d !! `datetime` instance

  type(datetime) :: d_utc
  integer        :: year, month, day, utc, hour, minute
  real(kind=real64)  :: second
  integer        :: A, Y, M, JDN
  ...
  date2num = date2num          &
   ...
   year   = d_utc % year                  ! Year
   month  = d_utc % month                 ! Month
   day    = d_utc % day                   ! Day
   utc    = d_utc % tz*60*60              ! Delta from UTC, convert from hours to seconds
   hour   = d_utc % hour                  ! Hour
   minute = d_utc % minute                ! Minute
   second = d_utc % second-utc+d_utc % millisecond/1000.0d0    ! Second   ! correction for time zone and milliseconds
...
!  Finding the Julian date given the JDN (Julian day number) and time of day
   date2num = JDN + dble(hour-12)/24.0d0 + dble(minute)/1440.0d0 + second/86400.0d0
 
end function  date2num

pure elemental type(datetime) function num2date(julian)

  !! Given a datetime instance d, returns Julian time.
  !! This is taken from http://fortranwiki.org/fortran/show/m_time as the
  !! original implementation involved a very crude loop over the number of years.

  real(kind=real64),intent(in) :: julian
  ...
  if(julian < 0)then
   ...
   second=dble((julian-dble(ijul))*s2d)       ! Seconds from beginning of Jul. Day

   if(second.ge.(s2d/2.0d0)) then             ! In next calendar day
      ijul=ijul+1
      second=second-(s2d/2.0d0)               ! Adjust from noon to midnight
   else                                       ! In same calendar day
      second=second+(s2d/2.0d0)               ! Adjust from noon to midnight
   endif

   if(second.ge.s2d) then                     ! Final check to prevent time 24:00:00
      ijul=ijul+1
      second=second-s2d
   endif
...
!=======================================================================
end module datetime_datetime_mod
